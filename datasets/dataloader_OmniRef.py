...
def load_OmniRef_json(split, image_root, dataset_path,extra_annotation_keys=None):


    
    ref_file = f'{dataset_path}/OmniRef.json'
    instances_file=f'{dataset_path}/instances.json'
    

    ref_data=json.load(open(ref_file, 'r'))[split]

    instances_datas = json.load(open(instances_file, 'r'))[split]

    # 将 annotations 列表转换为按 id 索引的字典，方便快速查找
    ann_id_to_data = {ann['id']: ann for ann in instances_datas['annotations']}
...
class OmniRefDataSet(Data.Dataset):
    def __init__(self, cfg,split):
        ...
        self.omniref_path=cfg.DATASETS.REF_ROOT
