...
condition=mask
model_name='msdatten_extract_supp_tokens_prompt_merge'
#OUTPUT_PATH=./outputs/${model_name}
RESUME='/data/zqc/code/ReLA_merge/outputs/msdatten_extract_supp_tokens_prompt_merge_18_9/model_last.pth'
OUTPUT_PATH=./outputs/msdatten_extract_supp_tokens_prompt_merge_18_9
mkdir -p ${OUTPUT_PATH}
mkdir -p ${OUTPUT_PATH}/${DATASET}

CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python -m torch.distributed.launch --nproc_per_node 8 --master_port 6224 train_net_merge_v3.py --model ${MODEL} \
        --dataset ${DATASET} --model_id ${DATASET} --batch-size  18 --pin_mem --print-freq 100 --workers 8 \
        --lr 1e-4 --wd 1e-2 --swin_type base --condition ${condition} --model_name ${model_name} --resume ${RESUME} \
        --warmup --warmup_ratio 1e-3 --warmup_iters 7500 --clip_grads --clip_value 0.01 \
        --pretrained_swin_weights ${SWIN_PATH} --epochs 70 --img_size ${IMG_SIZE} \
