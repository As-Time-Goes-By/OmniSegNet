...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # print(f"q type: {q.dtype}, shape: {q.shape}")
        # print(f"self.scale type: {type(self.scale)}, value: {self.scale}")
...
class MMBasicLayer(nn.Module):
    ...
    def forward(self, x, H, W, l, l_mask):
        ...
        # PWAM fusion
        x_residual = self.fusion(x, l, l_mask) if l is not None else x
...
class D2SwinTransformer(MultiModalSwinTransformer):
