...
from gres_model.modeling.prompt_encoder.transformer_v2 import TransformerDecoder,DeformableTransformerDecoderLayer
from gres_model.modeling.prompt_encoder.prompt import get_scribble_mask, get_point_mask, get_bounding_boxes
...
class GRES(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        num_prompts = 20
        self.vis_prompts = nn.Embedding(num_prompts, self.feat_feature)
        self.condition='mask'
    @classmethod
    ...
    def forward(self, ref_data={},supp_data={}):
        ...
        if ref_data.get('lang_tokens', None) is not None:
            ...
            ref_feat = None # (B, 768, N_l) to make Conv1d happy
            ref_mask = None  # (batch, N_l, 1)

        if supp_data.get('supp_image', None) is not None:
            supp_images = supp_data['supp_image']
            supp_masks=supp_data['supp_mask']
            un_supp_masks = supp_masks.unsqueeze(1)  # 从 [B, H, W] 扩展到 [B, 1, H, W]
            ...
            if self.condition == 'scribble':
                ...
                supp_masks = get_point_mask(supp_masks, self.training)  # point_mask
            ...
            supp_feat = visual_tokens.permute(0, 2, 1)  # (B, 768, N_l)
            supp_mask = visual_masks.unsqueeze(dim=-1)  # (batch, N_l, 1)
        else:
            supp_feat = None  # (B, 768, N_l) to make Conv1d happy
            supp_mask = None  # (batch, N_l, 1)
        ...
        # print('batch size',all_feat.shape[0])
        if self.training:
            batched_inputs={}
            if ref_feat is not None and supp_feat is not None:
                batched_inputs["gt_mask_merged"]=torch.cat([ref_data['gt_mask_merged'],supp_data['gt_mask_merged']],dim=0)
                batched_inputs["empty"] = torch.cat([ref_data['empty'], supp_data['empty']],dim=0)
            elif ref_feat is not None:
                batched_inputs["gt_mask_merged"] = ref_data['gt_mask_merged']
                batched_inputs["empty"] = ref_data['empty']
            elif supp_feat is not None:
                batched_inputs["gt_mask_merged"] = supp_data['gt_mask_merged']
                batched_inputs["empty"] = supp_data['empty']

            # targets = self.prepare_targets(batched_inputs, images)

            if ref_feat is not None:
                ref_targets = self.prepare_targets_v1({
                    "gt_mask_merged": ref_data['gt_mask_merged'],
                    "empty": ref_data['empty']
                }, ref_data['image'])
            if supp_feat is not None:
                supp_targets = self.prepare_targets_v1({
            ...
            if ref_feat is not None:
                # 计算参考任务的损失
                # ref_targets = {"gt_mask_merged": ref_data["gt_mask_merged"], "empty": ref_data["empty"]}
                ref_outputs = {}  # 对应参考任务的输出
            ...
            if supp_feat is not None:
                # 计算支撑任务的损失
                # supp_targets = {"gt_mask_merged": supp_data["gt_mask_merged"], "empty": supp_data["empty"]}
                # supp_outputs = outputs[-supp_data["gt_mask_merged"].shape[0]:]  # 对应支撑任务的输出
                supp_outputs = {}  # 对应参考任务的输出
    ...
    def prepare_targets_v1(self, batched_inputs, images):
