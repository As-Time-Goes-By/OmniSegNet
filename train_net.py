...
from datasets.dataloader_grefcoco import loader,RefCOCODataSet
from datasets.dataloader_OmniRef import OmniRefDataSet
from test import batch_evaluate

class ModelLoader:
    def __init__(self, args):
        self.model_use = args.model_name
        model_moudle_path = 'OmniSegNet_model.models.' + self.model_use + '.builder'
...
def train_one_epoch(model, optimizer, data_loader, lr_scheduler, epoch, print_freq, loss_scaler, clip_grad):
    ...
    for data in metric_logger.log_every(data_loader, print_freq, header):
        ...
        loss_dict = model(supp_data=data)
...
def main(args,distributed):
    ...
    for epoch in range(max(0, resume_epoch + 1), args.epochs):
        ...
        if epoch + 1:
            results = batch_evaluate(cfg,model, data_loader_test,cfg.DATASETS.DATASET_NAME)
