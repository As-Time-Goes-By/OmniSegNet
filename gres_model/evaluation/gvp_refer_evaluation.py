...
class GVPReferEvaluator(DatasetEvaluator):
    def __init__(
            self,
            dataset_name,
            distributed=True,
        ...
        self._available_sources = ["refcoco", "grefcoco", "gvpcoco"]
    ...
    def process(self, inputs, outputs,dataset_name):
        ...
        for i in range(batch_size):
            ...
            # 提取 GT 和预测 mask
            gt_mask = inputs['gt_mask_merged'][i].to(self._cpu_device)
            output_mask = outputs[i]["ref_seg"].argmax(dim=0).to(self._cpu_device)
            ...
            output_nt = outputs[i]["nt_label"].argmax(dim=0).bool().to(self._cpu_device)
